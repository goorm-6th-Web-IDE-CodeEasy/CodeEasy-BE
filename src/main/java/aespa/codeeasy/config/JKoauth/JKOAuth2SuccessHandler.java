...
@Component
public class JKOAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private final JKOAuth2AuthorizationRequestBasedOnCookieRepository authorizationRequestRepository;
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        // 1. 리프레시 토큰 생성 -> DB에 저장 -> 쿠키에 저장
        String refreshToken = tokenProvider.generateToken(member, REFRESH_TOKEN_DURATION);
        saveRefreshToken(member.getId(), refreshToken);
        addRefreshTokenToCookie(request, response, refreshToken);

        // 2. 엑세스 토큰 생성 -> 패스에 액세스 토큰 추가
    ...
    private void saveRefreshToken(Long userId, String newRefreshToken) {
        JKRefreshToken refreshToken = refreshTokenRepository.findByUserId(userId)
                .map(entity -> entity.update(newRefreshToken))
                .orElse(new JKRefreshToken(userId, newRefreshToken));
    ...
    private void addRefreshTokenToCookie(HttpServletRequest request, HttpServletResponse response, String refreshToken) {
        ...
        JKCookieUtil.deleteCookie(request, response, REFRESH_TOKEN_COOKIE_NAME);
        JKCookieUtil.addCookie(response, REFRESH_TOKEN_COOKIE_NAME, refreshToken, cookieMaxAge);
    }
